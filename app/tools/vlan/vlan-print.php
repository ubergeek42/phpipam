...
$vlans = $Tools->fetch_vlans_and_subnets ();
...
if($User->isadmin) {
	print "<a class='btn btn-sm btn-default' href='".create_link("administration","vlans")."' data-action='add'  data-switchid='' style='margin-bottom:10px;'><i class='fa fa-pencil'></i> ". _('Manage')."</a>";
}
...
foreach ($vlans as $vlan) {
	...
	# get section details
	$section = (array) $Sections->fetch_section (null, $vlan['sectionId']);
	...
	# check permission
	$permission = $Subnets->check_permission ($User->user, $vlan['subnetId']);
	if($permission>0) {
		...
		if ($vlan['subnetId'] != null) {
			# subnet
			print " <td><a href='".create_link("subnets",$section['id'],$vlan['subnetId'])."'>". $Subnets->transform_to_dotted($vlan['subnet']) ."/$vlan[mask]</a></td>";
			# section
	...
	if($User->settings->hideFreeRange!=1) {
		if($m==(sizeof($vlans)-1)) {
			if($User->settings->vlanMax > $vlan['number']) {
				print "<tr class='success'>";
				print "<td></td>";
				print "<td colspan='".(4+$csize)."'><btn class='btn btn-xs btn-default editVLAN' data-action='add' data-number='".($vlan['number']+1)."'><i class='fa fa-plus'></i></btn> "._('VLAN')." ".($vlan['number']+1)." - ".$User->settings->vlanMax." (".(($User->settings->vlanMax)-($vlan['number']))." "._('free').")</td>";
				print "</tr>";
			}
		}
	}
